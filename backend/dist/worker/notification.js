...
const notification_rules_1 = require("../config/notification-rules");
const QUEUE_NAME = 'notification_queue';
const MAX_RETRIES = 3;
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
async function processMessage(msg) {
    const { eventType, payload } = msg;
    const rule = notification_rules_1.NotificationRules[eventType];
    if (!rule) {
    ...
    const targetUserIds = [];
    if (rule.notifyDonor && payload.donorId)
        targetUserIds.push(payload.donorId);
    if (rule.notifyRecipient && payload.recipientId)
        targetUserIds.push(payload.recipientId);
    if (rule.notifyVolunteer && payload.volunteerId)
        targetUserIds.push(payload.volunteerId);
    ...
    for (const user of users) {
        ...
        if (rule.channels.includes('Email')) {
            await (0, email_1.sendEmail)(user.email, `FoodShare Update: ${eventType}`, payload.message || `You have a new update for your donation.`);
        }
        ...
        if (rule.channels.includes('SMS')) {
            ...
            await (0, sms_1.sendSMS)(phoneNumber, payload.message || `FoodShare Alert: ${eventType}`);
        }
    }
}
